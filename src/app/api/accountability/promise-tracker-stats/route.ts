...
/** Public KPI counts aligned with `GET /api/promises` query params (live browse strip). */
export async function GET(request: Request) {
  ...
  if (!(await allowPublicFormRequest(request, "promise-tracker-stats-read"))) {
    return NextResponse.json({ error: "Too many requests" }, { status: 429 });
  }

  const filters = parsePromisesApiFilters(new URL(request.url));
  const stats = await getCachedPromiseTrackerStats(filters);
