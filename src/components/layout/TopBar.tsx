...
const socialLinks = getTopBarSocialLinks();

function SocialIcon({ icon }: { icon: "facebook" | "linkedin" | "twitter" }) {
  if (icon === "facebook") {
    return (
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 sm:justify-end">
          ...
          <div className="flex items-center gap-2">
            {socialLinks.map(({ href, label, icon, external }) => {
              ...
              return (
                <Link key={icon} href={href} className={className} aria-label={`${label} — contact us for links`}>
                  {inner}
                </Link>
              );
            })}
