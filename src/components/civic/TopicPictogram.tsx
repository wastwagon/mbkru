import type { CitizenReportKind, ProgrammeEventKind } from "@prisma/client";

import type { PetitionTopicValue } from "@/lib/petition-topics";
...
/** Compact inline SVG markers for civic list cards — no photography required. */
export function TopicPictogram({
  ...
  const path = pictogramPath(variant);
  return (
    <span
      className={`inline-flex shrink-0 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--section-light)] text-[var(--primary)] ${className}`}
      aria-hidden
    >
      <svg className="h-[55%] w-[55%]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function pictogramPath(variant: PictogramVariant): string {
  switch (variant) {
    case "petition":
    case "OTHER":
    ...
    case "ENVIRONMENT":
      return "M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0h.5a2.5 2.5 0 002.5-2.5V3.935M21 12a9 9 0 11-18 0 9 9 0 0118 0z";
    case "SECURITY":
