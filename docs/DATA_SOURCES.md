...
| **`ndc-2024-manifesto-promises.catalogue.json`** | ~100 editorial abridged-manifesto **themes** (short lines), not a full rights-cleared extract of the PDF. **Does not populate the public dashboards by itself** — rows are created in Postgres only when you run `npx prisma db seed` with **`SEED_NDC_2024_MANIFESTO_CATALOGUE=1`** (and accountability seed **not** disabled with `SEED_ACCOUNTABILITY_DEMO=0`), plus a valid **John Dramani Mahama** starter MP. Without that opt-in, live pages show only the small hand-seeded sample. | Cross-check every row against the party’s canonical PDF; add `manifestoPageRef` in admin when signed off. |
