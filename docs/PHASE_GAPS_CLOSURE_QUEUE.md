...
- **F–M** communities **extensions** only (digest sends; standalone ban audit + appeals) — **baseline** communities + regions presence + **member + moderator** transactional outbox shipped; see granular rows in [`PHASE_TASKS.md`](./PHASE_TASKS.md) Phase 4+  
