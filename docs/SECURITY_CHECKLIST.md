...
- [ ] **Community transactional outbox:** member + **moderator** emails use **`MEMBER_TRANSACTIONAL_EMAIL`** jobs (Resend optional, same as report emails). Community SMS jobs use **`MEMBER_TRANSACTIONAL_SMS`** only when **`MBKRU_COMMUNITY_TRANSACTIONAL_SMS=1`** — keep off in staging unless you intend Twilio spend; **`POST /api/cron/notifications-outbox`** (or admin notifications retry) must run in production. Post-report path calls **`processNotificationOutboxBatch`** after enqueue for faster moderator alerts.
