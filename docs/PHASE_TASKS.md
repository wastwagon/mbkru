...
| [x] | **F–M (communities) — Delivery extension (members)** `NotificationDeliveryKind` **`MEMBER_TRANSACTIONAL_EMAIL`** / **`MEMBER_TRANSACTIONAL_SMS`** on `NotificationDeliveryJob`; Resend email (same **`RESEND_*`** as Voice) for join approved, post published/rejected, verification outcome, thread reply; optional SMS when **`MBKRU_COMMUNITY_TRANSACTIONAL_SMS=1`** and profile **`phone`** is E.164 (`SMS_PROVIDER` like Voice). Admin moderation + member replies call **`processNotificationOutboxBatch`** for faster delivery; cron still processes backlog |
| [x] | **F–M (communities) — Delivery extension (moderators)** Outbound email/SMS (same outbox + env gates) to **MODERATOR** + **QUEEN_MOTHER_VERIFIED** when a post is reported — **reporter excluded** from in-app + email/SMS; **`POST …/report`** drains outbox batch |
| [ ] | **F–M (communities) — Delivery digests** Batched / scheduled digests for community activity — *not implemented* |
