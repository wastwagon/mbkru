...
- **Rate limiting** — `POST` handlers for contact, newsletter, early access, tracker signup, **auth**, and **report submit / track** limit requests per IP; **GET** accountability JSON (`/api/mps`, `/api/promises`, phase 3 `/api/report-card/[year]`) uses the same helper per IP. Shared via **Redis** when `REDIS_URL` is set; otherwise in-memory for single-instance/dev.
- **Phase 2+ (`NEXT_PUBLIC_PLATFORM_PHASE` ≥ 2 at build)** — Public **member** auth (`/login`, `/register`, `/account`), **MBKRU Voice** `POST /api/reports` (optional **Turnstile** when `TURNSTILE_SECRET_KEY` is set), **attachments** `POST /api/reports/[id]/attachments`, **track** `GET /api/reports/track/[code]`, **`GET /api/mps`** (active roster JSON), **`GET /api/promises`**, **`/promises`** (browse campaign promises), **`/citizens-voice/submit`**, **`/track-report`**, **`/methodology`**, admin **Citizen reports**, **Parliament & promises** (CSV import). Requires **`MEMBER_SESSION_SECRET`** for member APIs; optional **`REPORT_ATTACHMENT_HMAC_SECRET`** (≥32 chars) enables anonymous attachment uploads via a short-lived token after submit.
