# MBKRU Platform Roadmap — Ghana 2028 Election

**Purpose:** Realistic timeline and implementation plan aligned with Ghana's next general election (November–December 2028).  
**Last updated:** March 2026

**Early recognition (sooner than these calendar quarters):** If you intend to **launch publicly earlier** for brand, partners, and pilots — without moving the 2028 election anchor — use **[`docs/EARLY_RECOGNITION_LAUNCH_PLAN.md`](docs/EARLY_RECOGNITION_LAUNCH_PLAN.md)** for **remaining gaps**, **R0–R4 recognition phases**, and how they relate to `NEXT_PUBLIC_PLATFORM_PHASE`.

---

## Executive Summary

MBKRU Advocates aims to position itself as Ghana's premier citizen accountability platform before the 2028 general election. This roadmap outlines phased implementation from Phase 1 (complete) through Phase 3 (election impact), with the **People's Report Card** and **Accountability Scorecards** (published 90 days before the election) as flagship deliverables.

**Key 2028 Milestone:** Accountability Scorecards published **August–September 2028** (90 days before election).

---

## Current State (March 2026)

### Phase 1 — Complete ✓

| Deliverable | Status |
|-------------|--------|
| Homepage | ✓ Live |
| About Us | ✓ Live |
| MBKRU Voice (Pillar A preview) | ✓ Preview page + Early Access form |
| Physical Engagement (Pillar B preview) | ✓ Preview page |
| Accountability (Pillar D preview) | ✓ Preview page + Tracker signup |
| News & Updates | ✓ Skeleton |
| Resources | ✓ Skeleton |
| Partners & Supporters | ✓ Live |
| Contact | ✓ Form + site copy (`site-content`) |
| Privacy & Terms | ✓ Legal templates |
| 16 Regions interactive viz | ✓ With modal data |

**Phase 1 does NOT include:** User registration, complaint submission, geo-tagged reporting, MP/Minister data, or live scorecards.

---

## Phased Implementation Timeline

### Phase 2 — Platform Build (Q2 2026 – Q2 2027)

**Goal:** Launch functional MBKRU Voice platform and begin physical engagement.

| Quarter | Milestone | Deliverables |
|---------|-----------|--------------|
| **Q2 2026** | Platform development | • MBKRU Voice MVP (registration, dashboard) • Town Hall pilot (Greater Accra) • Backend for complaint intake (no public submission yet) |
| **Q3 2026** | Early access launch | • Limited early access for registered members • Town Halls in Ashanti, Western, Central • Geo-tagged complaint pilot (Greater Accra) • Legal Empowerment Desk pilot |
| **Q4 2026** | National rollout | • Full MBKRU Voice launch (all 16 regions) • Regional Public Forums begin • First People's Report Card data collection starts • CHRAJ/FOI liaison established |
| **Q1 2027** | Full operations | • All five pillars operational • First Annual National People's Assembly • Public promise catalogue (2024 baseline rows) |
| **Q2 2027** | Data & scale | • People's Report Card Year 1 publication (baseline) • Constituency-level Town Halls • Parliamentary tracker data pipeline |

### Phase 3 — Election Readiness (Q3 2027 – Q4 2028)

**Goal:** Maximize impact on 2028 election cycle.

| Quarter | Milestone | Deliverables |
|---------|-----------|--------------|
| **Q3 2027** | Pre-election year prep | • People's Report Card Year 2 • Public promise catalogue live • Pre-election debate framework (constituency mapping) |
| **Q4 2027** | Election year ramp | • Accountability Scorecard methodology finalised • Constituency Town Hall schedule (275 constituencies) • Media partnerships for broadcast |
| **Q1 2028** | Campaign season | • Real-time catalogue updates (2028 manifestos) • Situational alerts for electoral issues • Legal Empowerment Desk: voter rights, dispute resolution |
| **Q2 2028** | Scorecard production | • Data aggregation for all MPs, Ministers, Regional Ministers • Scorecard drafting and review • Stakeholder preview (CSOs, media) |
| **Q3 2028** | **90-day publication** | • **Accountability Scorecards published (Aug–Sep 2028)** • Pre-election debates in every constituency • National People's Assembly (election focus) |
| **Q4 2028** | **Election** | • Ghana General Election (Nov–Dec 2028) • MBKRU as trusted accountability reference • Post-election catalogue updates begin |

---

## How MBKRU Impacts the 2028 Election

### 1. People's Report Card (Pillar D)

- **What:** Annual data-driven assessment of every Minister, Regional Minister, and MP.
- **Impact:** By 2028, three report cards (2026, 2027, 2028) will exist. Voters can compare incumbents' performance against tracked commitments in the public catalogue.
- **Timeline:** Year 1 (2027), Year 2 (2028), Year 3 (2028 pre-election).

### 2. Accountability Scorecards (Pillar D)

- **What:** Published **90 days before** every general election. Pre-election debates and town halls in every constituency.
- **Impact:** Directly shapes voter information. Candidates and incumbents are held to their records. Media and CSOs use scorecards for debates and coverage.
- **Timeline:** Publication **August–September 2028**.

### 3. Public promise catalogue (Pillar D)

- **What:** Real-time tracking of documented commitments versus delivery.
- **Impact:** 2024 promises become baseline. 2028 manifestos are tracked from day one. Voters see promise vs. delivery before casting ballots.
- **Timeline:** 2024 baseline by Q1 2027; 2028 manifestos from Q1 2028.

### 4. Physical Engagement (Pillar B)

- **What:** Quarterly Town Halls, Regional Forums, Annual National People's Assembly.
- **Impact:** Constituency-level debates and forums put accountability questions directly to candidates. National Assembly elevates citizen priorities to Presidency.
- **Timeline:** Constituency Town Halls in all 275 constituencies by Q3 2028.

### 5. MBKRU Voice (Pillar A)

- **What:** Digital platform for geo-tagged complaints, membership, and public statistics.
- **Impact:** Citizen-reported issues feed into People's Report Card and situational alerts. Demonstrates demand for accountability.
- **Timeline:** Full launch Q4 2026; scaled by 2028.

### 6. Legal Empowerment (Pillar C)

- **What:** CHRAJ, FOI, courts guidance. Pro-bono referrals. Voter rights support.
- **Impact:** Citizens can challenge electoral irregularities. FOI requests support data for scorecards. Legal support builds trust.
- **Timeline:** Pilot Q3 2026; full operations 2027.

### 7. Presidential Interface (Pillar E)

- **What:** Monthly Citizens' Brief, Quarterly Listening Sessions, liaison office.
- **Impact:** Citizen priorities reach Presidency. Demonstrates non-partisan bridge. Builds institutional credibility before election.
- **Timeline:** Liaison established 2027; Listening Sessions 2027–2028.

---

## Critical Path to 2028

```
2026 Q2  → MBKRU Voice MVP + Town Hall pilot
2026 Q4  → Full platform launch + Report Card data collection starts
2027 Q2  → People's Report Card Year 1 (baseline)
2027 Q4  → Accountability Scorecard methodology + constituency mapping
2028 Q2  → Scorecard data aggregation (all MPs, Ministers)
2028 Q3  → Accountability Scorecards published (90 days pre-election)
2028 Q4  → Ghana General Election
```

---

## Dependencies & Risks

| Dependency | Risk | Mitigation |
|------------|------|-------------|
| Funding / partnerships | Delayed platform build | Seek government, CSO, development partner funding early (2026) |
| Data access | Report Card quality | FOI requests; CHRAJ liaison; parliamentary data partnerships |
| Technical capacity | MVP delays | Phase 2 requires dedicated dev team; consider outsourced build |
| Political neutrality | Credibility | Strict non-partisanship; transparent methodology; CSO validation |
| Electoral Commission | Debate/forum coordination | Early engagement with EC on civic education role |

---

## Recommended Next Steps (Immediate)

1. **Engineering spec** — Implementation checklist, research references, data model, and UI charter: [`docs/PHASES_2_3_IMPLEMENTATION.md`](docs/PHASES_2_3_IMPLEMENTATION.md).
2. **Secure partnerships** — Government, CHRAJ, media, development partners.
3. **Hire or contract** — Technical lead for platform development (Phase 2 auth + reporting + moderation).
4. **Town Hall pilot** — Confirm Greater Accra pilot venue and date (Q2 2026).
5. **Update website roadmap** — Keep the homepage roadmap section aligned with this document by editing `programmeRoadmap` in `src/lib/site-content.ts` when milestones change.

---

## Appendix: Ghana Election Context

- **2028 election:** November or December 2028 (EC has proposed move to November).
- **90-day rule:** Accountability Scorecards published ~August–September 2028.
- **Constituencies:** 275 parliamentary constituencies.
- **Regions:** 16 administrative regions.
- **Key offices tracked:** President, Ministers, Regional Ministers, MPs (275).
